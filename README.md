...
#### 1.1.1. Entropy Accumulator

The **Entropy Accumulator** collects and stores randomness in the form of a  **256-bit hash value**. To do this, it uses the **SHAd-256 hashing function**, which takes the raw entropy (random data), processes it, and  **compresses it into a fixed-size 256-bit hash**. This hashed output is then saved into the entropy pools.

#### 1.1.2. Key Generator

The **Key Generator** module is responsible for producing random cryptographic keys for the Generator core. It gathers entropy from all pools during a key-reseed operation and feeds this data into a **SHAd-256 hashing engine**. The hashing engine mixes and compresses the collected entropy into a **256-bit hash**, which becomes the newly generated key.

#### 1.1.3. Generator Core (RNG Generator)

The **Generator core** is the heart of Fortuna, it will produce the random number by utilizing seed as the encryption key and counter 128-bit as the plaintext. It will generate 128-bit random number. In this design, **AES-256** is used as the primary generator core, with **ChaCha** and **Salsa20** provided as alternative cipher core implementations.

### 1.2 Fortuna Accelerator Advantage

* Able to accumulate the input data (improve randomness)
* Extremely fast compared to SW routine
* Produce a high volume of random data
* Cryptographically secure : data generated is nearly impossible to predict
...
- [X] Introduce Cipher Selector Logic
- [X] Performance matrix achieved
...
- [ ] Modify Datapath structure to fully match Fortuna cryptographic specs (i.e. implement stored seed data as non volatile memory, slightly modify its size and the way it is used at the moment).
- [ ] Try to speed up data generation (try to reduce number of cycles, increase FPGA max frequency, study tradeoff between both).
...
- **Sources**: `Agile-HW-Design-RNG/src/main/scala/`
  ...
  - `Datapath.scala` - Datapath for Fortuna that orchestrates how entropy moves through from Seed Generator to Generator Core.
...
Alternative cipher module for generator core. Module implementation is based on [ChaCha by  Daniel J. Bernstein](https://cr.yp.to/chacha/chacha-20080128.pdf).
...
- IO (file: `Agile-HW-Design-RNG/src/main/scala/ChaCha.scala`)
  ...
  - `io.out: UInt(512.W)` - The 512‑bit ChaCha keystream block
  - `io.done: Bool` - Done or valid flag

The figure 2.2 gives the initial internal state of the ChaCha Module. This is later changes and diffuses through a quartet function, This happens 10 x 2 times a column - diagonal shift pair.
...
<p align="center">Figure 2.3: ChaCha Quartet Function.</p>
...
The figure 2.4  gives the initial internal state of the Salsa20  Module. This later changes and diffuses through a quartet function, This also happens 10 x 2 times a column - row shift pair.

<div align="center">
<img src="image/README/initial_state_Salsa.png" alt="ini_salsa">
</div>

<p align="center">Figure 2.4: Initial state Matrix of Salsa20 Core.</p>

The figure 2.5 gives the quartet function for the Salsa20 Module.

<div align="center">
<img src="image/README/Quartet_salsa.png" alt="sal_quat">
</div>

<p align="center">Figure 2.5: Salsa20 Quartet Function.</p>
...
- Run a specific test e.g: `sbt "testOnly *SHA256Tester"`, `sbt "testOnly *SHAd256Tester"`, or `sbt "testOnly *AES256Tester"`.
...
The design was implemented on a Nexys 4DDR/ Artix-7 (A7) FPGA development board. The SystemVerilog files used for the implementation are located under `Agile-HW-Design-RNG/fpga` folder.
...
The design uses about **41%** of the available slice LUTs and **15%** of slice registers of Nexys A7 FPGA. Figure 3.2 illustrates details of the resource utilization for the design.
...
- **Data generation** :  3 data blocks generated ( 1 for output, 2 to update the key)
  - AES  version : 3 * 15  = **45 cycles** for  128 bits generated
  - ChaCha/Salsa version :  3 * 12  = **36 cycles** for 512 bits generated
- **Adding data** to the pools : 1 run of SHAd256 module, i.e. **70 cycles**
- **Reseeding cipher key** with pools data : 1 run of SHAd256 module, i.e. **70 cycles * # pools used for reseed**
- **Average number of pools** used for reseed : 2 → average # cycles : **140 cycles**
- **Generation speed** (considering only the data generation cycles)
  - AES version : around **55 Mbits/s**
  - ChaCha / Salsa: around **320 Mbits/s**

### 3.4 Timing Report

Figure 3.3 reports the timing that shows the critical clock paths inside the design. Each path has a total delay around **52-53 ns**, so that the maximum frequency would be **18.7 MHz**.
