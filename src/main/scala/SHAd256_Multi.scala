...
class SHAd256_Multi extends Module {
  ...
  val shift = 32.U - cnt * 512.U
  val shiftBlocks = io.shaBlocks >> shift
  val slice = shiftBlocks(512, 0)


  switch(state) {
    is(sIdle) {
      when(io.start || cnt > 0.U) {
        for(i <- 0 until 8) { H(i) := Hinit(i) }
        for(i <- 0 until 16) { W(i) := slice(512 - i*32 , 480 - i*32) }
