...
class SHA256Tester extends AnyFlatSpec with ChiselScalatestTester {
  behavior of "SHA256"

  it should "hash a single 512-bit block" in {
    test(new SHA256()).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
